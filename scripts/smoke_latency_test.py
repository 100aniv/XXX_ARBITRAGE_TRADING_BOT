...
def main():
    ...
    print(f"\n[OK] Optimization Priority: {report.optimization_priority}")
    ...
    if redis_read_count == 0 or redis_write_count == 0:
        print("\n[FAIL] Redis latency not measured (count=0)")
        sys.exit(1)
    
    if db_count == 0:
        print("\n[FAIL] DB latency not measured (count=0)")
        sys.exit(1)
