#!/usr/bin/env python3
"""
Gate 10분 테스트 SSOT (Single Source of Truth)

목적:
- 600초 실행 + exit code 0 + KPI JSON 생성을 강제
- 모든 예외 상황에서도 KPI JSON 생성 보장
- 재현 가능한 증거 생성 (logs, kpi.json, 커맨드)

Exit Code:
- 0: PASS (600초 이상 실행, exit 0, kpi.json 생성 성공)
- 1: FAIL (위 조건 중 하나라도 미충족)
"""
