...
# -*- coding: utf-8 -*-
"""
D74-2.5: Extended Multi-Symbol PAPER Soak Runner
Top-10 PAPER 모드 60분 soak 캠페인 - 롱 런 베이스라인 측정

Purpose:
- D74-2(10분 baseline)를 시간 연장하여 "롱 런 멀티심볼 동작 안정성" 검증
- D74-3 성능 최적화 이후에도 비교할 수 있는 60분 soak 베이스라인 확보
- 더 큰 표본(거래 수/심볼 분포/스프레드 패턴) 수집
- D74-2와 D74-3 사이에 삽입되는 추가 단계

Usage:
    python scripts/run_d74_2_5_paper_soak.py
    python scripts/run_d74_2_5_paper_soak.py --duration-minutes 60
    python scripts/run_d74_2_5_paper_soak.py --duration-minutes 30 --log-level INFO
"""
...
# 로깅 설정
log_filename = f'logs/d74_2_5_paper_soak_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'
logging.basicConfig(
    level=logging.INFO,
...
def create_d74_2_5_config(duration_minutes: float = 60.0) -> ArbitrageConfig:
    """
    D74-2.5 Extended PAPER Soak용 ArbitrageConfig 생성
    ...
    logger.info(f"[D74-2.5] Config created: TOP_N={config.universe.top_n}, runtime={max_runtime}s ({duration_minutes:.1f}min)")
    return config
...
def setup_paper_exchanges() -> tuple[PaperExchange, PaperExchange]:
    ...
    logger.info("[D74-2.5] Paper exchanges created with sufficient balance")
...
async def run_d74_2_5_campaign(
    config: ArbitrageConfig,
    duration_minutes: float = 60.0,
) -> Dict[str, Any]:
    """
    D74-2.5 Extended PAPER Soak 캠페인 실행
    ...
    """
    logger.info(f"\n{'='*80}")
    logger.info("D74-2.5: Extended Multi-Symbol PAPER Soak Campaign")
    ...
    logger.info("[D74-2.5] Creating MultiSymbolEngineRunner...")
    ...
    logger.info(f"[D74-2.5] Universe symbols ({len(symbols)}): {symbols}")
    
    # 4. RiskCoordinator 확인
    if runner.risk_coordinator:
        risk_stats = runner.risk_coordinator.get_stats()
        logger.info(f"[D74-2.5] RiskCoordinator initialized: {risk_stats}")
    
    # 5. 캠페인 실행
    max_runtime = int(duration_minutes * 60)
    logger.info(f"\n[D74-2.5] Starting {duration_minutes:.1f}min soak campaign (max_runtime={max_runtime}s)...\n")
    ...
    try:
        ...
        logger.info(f"\n[D74-2.5] Campaign completed in {runtime:.2f}s ({runtime/60:.2f}min)")
        
        # 6. Final RiskCoordinator 통계
        if runner.risk_coordinator:
            final_risk_stats = runner.risk_coordinator.get_stats()
            logger.info(f"\n[D74-2.5] Final RiskCoordinator Stats:")
            for key, value in final_risk_stats.items():
                logger.info(f"  {key}: {value}")
            stats["risk_stats"] = final_risk_stats
        
        # 7. PaperExchange 체결 분석
        logger.info("\n[D74-2.5] Analyzing Paper Exchange trades...")
        
        trade_stats_a = analyze_paper_exchange_trades(exchange_a)
        trade_stats_b = analyze_paper_exchange_trades(exchange_b)
        
        logger.info(f"[D74-2.5] Exchange A trades: {trade_stats_a['filled_orders']} filled, {len(trade_stats_a['symbols_with_trades'])} symbols")
        logger.info(f"[D74-2.5] Exchange B trades: {trade_stats_b['filled_orders']} filled, {len(trade_stats_b['symbols_with_trades'])} symbols")
        ...
        logger.error(f"[D74-2.5] Campaign failed: {e}", exc_info=True)
        return {
            "error": str(e),
...
def print_summary(stats: Dict[str, Any]) -> None:
    ...
    """
    print(f"\n{'='*80}")
    print("D74-2.5 Extended PAPER Soak Campaign Summary")
    print(f"{'='*80}")
    
    if "error" in stats:
        print(f"[FAIL] Campaign failed: {stats['error']}")
        return
...
def check_acceptance_criteria(stats: Dict[str, Any], duration_minutes: float = 60.0) -> bool:
    """
    D74-2.5 Acceptance Criteria 체크
    
    Args:
        stats: 실행 통계 dict
        duration_minutes: 예상 실행 시간 (분)
    
    Returns:
        True if all criteria met, False otherwise
    """
    print(f"\n{'='*80}")
    print("D74-2.5 Acceptance Criteria Check")
    print(f"{'='*80}\n")
    ...
    # 2. Runtime tolerance: 60분 ±2%
    actual_minutes = stats.get('actual_runtime_minutes', 0)
    expected_minutes = duration_minutes
    tolerance_pct = 2.0
    lower_bound = expected_minutes * (1 - tolerance_pct/100)
    upper_bound = expected_minutes * (1 + tolerance_pct/100)
    
    if lower_bound <= actual_minutes <= upper_bound:
        print(f"[PASS] Duration: {actual_minutes:.2f}min within {expected_minutes}min ±{tolerance_pct}% ({lower_bound:.2f}~{upper_bound:.2f})")
    else:
        print(f"[WARN] Duration: {actual_minutes:.2f}min outside tolerance {lower_bound:.2f}~{upper_bound:.2f}min")
    
    # 3. 최소 20개 심볼에서 체결 (KRW + USDT)
    traded_symbols = stats.get('total_traded_symbols', 0)
    if traded_symbols >= 20:
        print(f"[PASS] Traded symbols: {traded_symbols} >= 20 (KRW+USDT)")
    else:
        print(f"[FAIL] Traded symbols: {traded_symbols} < 20")
        passed = False
    
    # 4. 최소 2,000건 체결
    total_filled = stats.get('total_filled_orders', 0)
    if total_filled >= 2000:
        print(f"[PASS] Total filled orders: {total_filled} >= 2,000")
    else:
        print(f"[FAIL] Total filled orders: {total_filled} < 2,000")
        passed = False
    
    # 5. RiskGuard decision 최소 1회 이상 기록
    if "risk_stats" in stats:
        risk = stats["risk_stats"]
        total_dec = risk.get('total_decisions', 0)
        
        if total_dec >= 1:
            print(f"[PASS] RiskGuard decisions recorded: {total_dec} >= 1")
        else:
            print(f"[WARN] No RiskGuard decisions recorded (expected >=1 for latency analysis)")
    else:
        print("[WARN] No RiskGuard stats available")
    
    # 6. PaperExchange fill 정상 동작
    exch_a_filled = stats.get('exchange_a_trades', {}).get('filled_orders', 0)
    exch_b_filled = stats.get('exchange_b_trades', {}).get('filled_orders', 0)
    
    if exch_a_filled > 0 and exch_b_filled > 0:
        print(f"[PASS] PaperExchange fill working: A={exch_a_filled}, B={exch_b_filled}")
    else:
        print(f"[FAIL] PaperExchange fill issue: A={exch_a_filled}, B={exch_b_filled}")
        passed = False
...
def main():
    """메인 함수"""
    parser = argparse.ArgumentParser(description="D74-2.5 Extended Multi-Symbol PAPER Soak Test")
    parser.add_argument(
        "--duration-minutes",
        type=float,
        default=60.0,
        help="Campaign duration in minutes (default: 60.0)"
    )
    parser.add_argument(
        "--log-level",
        type=str,
    ...
    logger.info(f"[D74-2.5] Starting Extended PAPER Soak Campaign")
    logger.info(f"[D74-2.5] Log file: {log_filename}")
    
    # Config 생성
    config = create_d74_2_5_config(duration_minutes=args.duration_minutes)
    
    # 캠페인 실행
    stats = asyncio.run(run_d74_2_5_campaign(
        config=config,
    ...
    passed = check_acceptance_criteria(stats, duration_minutes=args.duration_minutes)
