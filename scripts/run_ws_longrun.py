# -*- coding: utf-8 -*-
"""
D52 WebSocket Long-run Runner

WebSocket 기반 롱런 테스트 실행 스크립트.
"""
...
def load_config(config_path: str) -> dict:
    ...
    logger.info(f"[D52_CLI] Loaded config from {config_path}")
    return config
...
def create_exchanges(config: dict, mode: str):
    ...
    logger.info("[D52_CLI] Created Paper exchanges: A and B")
...
def create_engine(config: dict) -> ArbitrageEngine:
    ...
    logger.info(f"[D52_CLI] Created ArbitrageEngine with config: {arb_config}")
    return engine
...
def create_live_config(
    ...
    live_cfg = ArbitrageLiveConfig(
        ...
        data_source=data_source,  # D52: 강제 ws
    )
    
    logger.info(f"[D52_CLI] Created ArbitrageLiveConfig: {live_cfg}")
    return live_cfg
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="D52 WebSocket Long-run Runner"
    )
    ...
    try:
        logger.info(
            f"[D52_CLI] Starting WebSocket Long-run: "
        ...
        # D52: WebSocketMarketDataProvider 생성 (ws-only)
        # 실제 WS 어댑터는 config에서 로드됨
        ws_config = config.get("ws", {})
        market_data_provider = WebSocketMarketDataProvider(ws_adapters={})
        logger.info("[D52_CLI] Created WebSocketMarketDataProvider (forced for WS longrun)")
        
        # D52: MetricsCollector 생성
        metrics_collector = MetricsCollector(buffer_size=300)
        logger.info("[D52_CLI] Created MetricsCollector")
        ...
        logger.info(
            f"[D52_CLI] Starting WebSocket Long-run in paper mode: "
        ...
        logger.info(f"[D52_CLI] Final stats: {json.dumps(stats, indent=2)}")
        
        # 메트릭 요약
        metrics = metrics_collector.get_metrics()
        logger.info(f"[D52_CLI] Final metrics: {json.dumps(metrics, indent=2)}")
        
        # 최종 리포트 출력
        print("\n" + "=" * 70)
        print(f"🎯 D52 WebSocket Long-run Report - Scenario {args.scenario}")
        print("=" * 70)
        ...
        if metrics:
            ...
            print(f"  WS Connected: {metrics.get('ws_connected', False)}")
            print(f"  WS Reconnects: {metrics.get('ws_reconnect_count', 0)}")
        ...
        logger.error(f"[D52_CLI] Error: {e}", exc_info=True)
