...
class PreflightChecker:
    ...
    def check_redis(self, runner: PaperRunner) -> bool:
        ...
        try:
            ...
            if not hasattr(runner, 'redis_client') or not runner.redis_client:
                self.log("  ⚠️  WARN: redis_client not initialized (optional mode)")
            ...
            self.log(f"  ⚠️  WARN: Redis check failed: {e}")
    ...
    def run_smoke(self, duration_minutes: float = 1.0) -> bool:
        ...
        try:
            ...
            # Smoke 실행
            self.log(f"  ▶️  Starting {duration_minutes}min smoke...")
            ...
            if runner.kpi.closed_trades > 0:
                ...
                self.log(f"  ⚠️  No trades executed (market conditions)")
