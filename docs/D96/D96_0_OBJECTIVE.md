...
D96은 TopN 확장 Gate로, Top20에서 Top50으로 확장 시 안정성과 부하 처리를 검증한다.

**핵심 목표**:
- TopN 확장 (Top20 → Top50)
- 부하/레이트리밋/헬스 기반 안정성 검증
...
├── d96_top50_1h_kpi.json    # 1h baseline KPI (예정)
└── d96_log_tail.txt         # 로그 tail
```
...
python scripts/run_d77_0_topn_arbitrage_paper.py --universe top50 --duration-minutes 60 --kpi-output-path docs/D96/evidence/d96_top50_1h_kpi.json
```
