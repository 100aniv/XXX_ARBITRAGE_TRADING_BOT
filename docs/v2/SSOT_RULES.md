...
### 2. 수익성 기준 먼저, 레이턴시 최적화는 나중

**원칙:** D205-10(수익성)이 D205-11(레이턴시)보다 먼저 완료되어야 함

**강제 규칙:**
1. **D205-10 (Profitability) 완료 전:** D205-11 (Latency) 진입 금지
2. **수익성 검증:** break-even threshold 재정의 + 비용 모델 적용
3. **레이턴시 목표:** 수익성 기준에 맞춘 p95 < 100ms

**근거:** 빠르지만 수익 없는 시스템은 무의미. 수익성이 먼저 확보되어야 레이턴시 최적화 의미 있음

### 3. Record/Replay 없으면 튜닝/회귀 불가
