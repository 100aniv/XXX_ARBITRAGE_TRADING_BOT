"""
D202-1: Binance WebSocket Provider (reconnect 포함)
...
class BinanceWsProvider(WsProvider):
    """
    Binance WebSocket Provider (L2 Orderbook)
    
    API 문서: https://binance-docs.github.io/apidocs/spot/en/#websocket-market-streams
    ...
    async def connect(self) -> None:
        """WebSocket 연결 (mock)"""
        logger.info("[D202-1_BINANCE_WS] Connecting to WebSocket (mock)")
        self._is_connected = True
        self._reconnect_count = 0
    
    async def disconnect(self) -> None:
        """WebSocket 연결 종료"""
        logger.info("[D202-1_BINANCE_WS] Disconnecting WebSocket")
        self._is_connected = False
        if self.ws:
            self.ws = None
    ...
    async def subscribe(self, symbols: List[str]) -> None:
        ...
        Args:
            symbols: ["BTC/USDT", "ETH/USDT"] 형식
        """
        logger.info(f"[D202-1_BINANCE_WS] Subscribing to symbols: {symbols}")
    ...
    async def _reconnect(self) -> bool:
        ...
        if self._reconnect_count >= self.max_reconnect_attempts:
            logger.error(
                f"[D202-1_BINANCE_WS] Max reconnect attempts reached: "
        ...
        logger.warning(
            f"[D202-1_BINANCE_WS] Reconnecting... "
        ...
        try:
            await self.connect()
            logger.info("[D202-1_BINANCE_WS] Reconnect successful")
            return True
        except Exception as e:
            logger.error(f"[D202-1_BINANCE_WS] Reconnect failed: {e}")
